...
function EditProfileModal({ visible, onClose, user, onSave, saving }) {
...
function ChangePasswordModal({ visible, onClose, onSave, saving }) {
...
export default function ProfileScreen() {
  ...
  const refreshProfile = useCallback(async () => {
    try {
      ...
      // keep the existing local state when the API is temporarily unavailable
    }
  }, [updateUser]);
  ...
  const persistProfileUpdate = useCallback(
    async (changes, successMessage) => {
      setProfileSaving(true);
      try {
        const result = await usersAPI.updateMe(changes);
        if (result?.user) {
          updateUser(result.user);
        }
        if (successMessage) {
          Alert.alert('✅ נשמר!', successMessage);
        }
        return true;
      } catch (err) {
        Alert.alert('שגיאה', err.message || 'לא ניתן לעדכן את הפרופיל כרגע');
        return false;
      } finally {
  ...
  const handleEditProfileSave = async ({ name, age, height }) => {
    const nextAge = age ? parseInt(age, 10) : undefined;
    const nextHeight = height ? parseInt(height, 10) : undefined;
    const success = await persistProfileUpdate(
      {
        name,
        age: Number.isFinite(nextAge) ? nextAge : undefined,
  ...
  const handleQuickProfileUpdate = async (changes, successMessage) => {
    await persistProfileUpdate(changes, successMessage);
  };
  ...
  const handleChangePassword = async (currentPassword, newPassword) => {
    ...
    try {
      ...
      Alert.alert('✅ נשמר!', 'הסיסמה שונתה בהצלחה');
    } catch (err) {
      Alert.alert('שגיאה', err.message || 'לא ניתן לשנות את הסיסמה כרגע');
    } finally {
  ...
  const handleDeleteAccount = () => {
    Alert.alert('מחיקת חשבון', 'פעולה זו אינה ניתנת לביטול. כל הנתונים שלך יימחקו.', [
      { text: 'ביטול', style: 'cancel' },
      {
        text: 'מחק חשבון',
        style: 'destructive',
  ...
  const currentNotif =
    NOTIFICATION_LEVELS.find(l => l.value === notificationLevel)?.label || 'כל ההתראות';
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <PickerModal
        ...
        options={GOALS}
        selected={user.goal}
        onSelect={value => handleQuickProfileUpdate({ goal: value }, 'המטרה עודכנה בהצלחה')}
      />
      <PickerModal
        ...
        onSelect={value =>
          handleQuickProfileUpdate({ activityLevel: value }, 'רמת הפעילות עודכנה בהצלחה')
        }
      />
      <PickerModal
